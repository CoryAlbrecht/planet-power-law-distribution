...
def compute_surface_gravity(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df["mass_kg"] = M
    df["mass_kg_err1"] = df["pl_bmassjerr1"].fillna(0) * M_JUP_KG
    df["mass_kg_err2"] = df["pl_bmassjerr2"].fillna(0) * M_JUP_KG

    df["radius_m"] = R
    df["radius_m_err1"] = df["pl_radjerr1"].fillna(0) * R_JUP_M
    df["radius_m_err2"] = df["pl_radjerr2"].fillna(0) * R_JUP_M

    df["surf_grav_ms2"] = g.round(4)
    df["surf_grav_earth"] = (g / G_EARTH).round(4)

    M_err1 = df["mass_kg_err1"]
    M_err2 = df["mass_kg_err2"]
    R_err1 = df["radius_m_err1"]
    R_err2 = df["radius_m_err2"]
    ...
    df["surf_grav_ms2_err1"] = (g_large - g).round(4)
    df["surf_grav_ms2_err2"] = (g - g_small).round(4)
    df["surf_grav_earth_err1"] = (df["surf_grav_ms2_err1"] / G_EARTH).round(4)
    df["surf_grav_earth_err2"] = (df["surf_grav_ms2_err2"] / G_EARTH).round(4)
...
def assign_dm_class(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df["dm_grav_residual"] = (df["surf_grav_earth"] - df["dm_pred_g_earth"]).round(4)
